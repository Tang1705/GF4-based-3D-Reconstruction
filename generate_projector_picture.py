...
if __name__ == '__main__':
    ...
    # c.show()
    # c.save("9.28a.png")
    d = Image.new("RGB", (x, y), bgcolor)
    image_resize = c.resize((x // 2, y), Image.ANTIALIAS)
    # image_resize.show()
    # image_resize.save("9.28a.png")
    d = Image.new("RGB", (x, y), bgcolor)
    image_resize = c.resize((x // 2, y), Image.ANTIALIAS)
    cv_image = cv2.cvtColor(np.array(image_resize), cv2.COLOR_RGB2BGR)
    start = end = 0
    start = (912 - image_resize.width) // 2
    end = start + image_resize.width
    i = 0
    while i < y:
        j = 0
        while j < x:
            if j > start and j < end:
                d.putpixel([j, i], (cv_image[i][j - start][2], cv_image[i][j - start][1], cv_image[i][j - start][0]))
            j = j + 1
        i = i + 1
    d.show()
    d.save("projector.png")
